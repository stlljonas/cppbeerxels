    ...
    for (int j = 0; j < _numberOfNodesY; ++j) { // iterate over y direction
        for (int i = 0; i < _numberOfNodesX; ++i) {
            if (j%2 == 0) { // even row, starting at idx 0 
                ...
                // x=
                // y= 
                cv::Point point(xBorder + 2.0*static_cast<float>(i*_radius),yBorder + sqrt(3)*static_cast<float>(j*_radius));
                _nodes.push_back(point);
            } else if (!(_everySecondRowOneNodeLess && i == _numberOfNodesX - 1)) { // odd row and no edge case
                std::cout << "odd" << std::endl;
                cv::Point point(xBorder + (2*i+1)*_radius,yBorder + sqrt(3)*j*_radius);
                _nodes.push_back(point);
            }
            if (_everySecondRowOneNodeLess && j%2 == 1 && i == _numberOfNodesX - 1) continue;
            
        }
    } */
}

std::vector<cv::Point> HoneyCombTiling::getNodes() {
    return _nodes;
}

int HoneyCombTiling::getRadius() {
    return _radius;
}
...
std::vector<cv::Point> HoneyCombTiling::_tile(int radius) {
    ...
    const float xStep = 2.0 * static_cast<float>(radius);  // step size in x direction
    // cos(30) = h/(2r) = sqrt(3)/2
    // h = sqrt(3)*r
    const float yStep = sqrt(3) * static_cast<float>(radius);  // step size in y direction
    ...
    _overhangY = static_cast<int>(yRef - (2.0 + r * (sqrt(3.0)*(yNum - 1.0))));  // todo
...
bool HoneyCombTiling::_circleIsInBound(int x, int y, int radius) {
    if (x - radius < 0 || y - radius < 0) {
        //std::cout << "out\n";
        return false;
    } else if (x + radius >= _referenceXDimension || y + radius >= _referenceYDimension) {
        //std::cout << "out\n";
        return false;
    } else {
        //std::cout << "in\n";
